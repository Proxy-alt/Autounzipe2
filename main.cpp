...
public:
    ...
    void InitializePaths() {
        ...
        if (peazipPath.empty() || !std::filesystem::exists(peazipPath)) {
            std::vector<std::string> possiblePaths = {
                ...
                GetCurrentDirectory() + "\\PeaZip\\peazip.exe"
            };
    ...
    void LogEvent(const std::string& message) {
        std::string logPath = GetCurrentDirectory() + "\\AutoUnzipService.log";
            ...
            case WM_COMMAND:
                switch (LOWORD(wParam)) {
                    ...
                    case ID_TRAY_SHOW: {
                        ...
                        status += "Log File: " + service->GetCurrentDirectory() + "\\AutoUnzipService.log";
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    AutoUnzipService tempService;
    std::string tempPeaZipPath;
    
    // Get PeaZip path using the same logic as the service
    HKEY hKey;
    char buffer[MAX_PATH];
    DWORD bufferSize = sizeof(buffer);
